...
export async function seed(knex: Knex): Promise<void> {
  ...
  const companiesMap = companies.reduce(
    (mapping, company) => mapping.set(company.company_name, company.id),
  ...
  const vehiclesMap = vehicles.reduce(
    (mapping, vehicle) => mapping.set(vehicle.car_plate, vehicle.id),
    new Map<string, number>()
  );
  ...
  const devicesMap = devices.reduce(
    (mapping, device) => mapping.set(device.device_eui, device.id),
    new Map<string, number>()
  );
