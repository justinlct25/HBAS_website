...
export async function seed(knex: Knex): Promise<void> {
  ...
  const companies: Array<InsertCompanies> = await knex('companies')
  ...
  const vehicles: Array<InsertVehicles> = await knex('vehicles')
  ...
  const devices: Array<InsertDevices> = await knex('devices')
